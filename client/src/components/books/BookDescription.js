...
class BookDescription extends React.Component {
  ...
  bookDescriptionList = book => {
    return (
      <List>
        <List.Item as="h3">{book.title}</List.Item>
        {book.authors && (
          <List.Item>
            <strong>Written by</strong> {book.authors[0]}
          </List.Item>
        )}
        <Divider />
      ...
      <Grid as={PageContain}>
        <Grid.Row columns={2}>
          <Grid.Column id="desc-container">
            <Segment basic>{this.bookDescriptionList(book)}</Segment>
          </Grid.Column>
        ...
        <Grid.Row centered>
          {!this.isLoggedIn() && (
            <em>
              <Link to="/login">Sign in</Link> to add this book to your
              bookshelf!
            </em>
          )}
          {this.isLoggedIn() && <BookshelfDropdown />}
          {this.isLoggedIn() && (
            <Icon name="book" onClick={() => this.toggleEdit()} />
          )}
        </Grid.Row>
