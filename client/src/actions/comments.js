  ...
  return dispatch => {
    axios
      ...
      .catch(err =>
        dispatch(
          setFlash(
            `Could not create comment topic, please try again!`,
            'red'
          )
        )
      );
...
export const fetchComment = () => {
  return dispatch => {
    axios
      .get('/api/comments/', setHeaders())
      .then(res => dispatch({ type: 'GET_COMMENT', comment: res.data }))
      .catch(err =>
        setFlash(
          'Could not retrieve comment topics, please try again!',
          'red'
        )
      );
  };
